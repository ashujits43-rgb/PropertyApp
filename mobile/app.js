...
axios.defaults.baseURL = "https://propertyapp-n2fh.onrender.com";
...
const Login = ({ navigation }) => {
  ...
  const login = async () => {
    if (!/^\d{10}$/.test(phone)) return Alert.alert("Error", "Enter valid phone");
    ...
    try {
      ...
      Alert.alert("Failed", e.response?.data?.msg || "Backend offline");
    }
  };
...
const Home = ({ navigation }) => {
  ...
  const load = async () => {
    try {
      ...
      const res = await axios.get("/api/properties/my", {
        headers: { "x-auth-token": token }
      });
      setProperties(res.data);
    } catch (e) {
      Alert.alert("Error", "Login again or backend offline");
    }
  };
...
const AddProperty = ({ navigation }) => {
 ...
 const [location, setLocation] = useState("");
  ...
  const save = async () => {
    if (!title || !price || !location) return Alert.alert("Error", "Fill all fields");

    try {
      const token = await AsyncStorage.getItem("token");
      await axios.post("/api/properties",
        { title, price: Number(price), location },
        { headers: { "x-auth-token": token } }
      );
